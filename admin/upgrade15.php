...
if ($step==1) {
  ...
  <li>Upgrading from vBulletin 2.0 beta 1: run <i><? echo createupgradelist(2,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 beta 2: run <i><? echo createupgradelist(3,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 beta 3: run <i><? echo createupgradelist(4,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 beta 4: run <i><? echo createupgradelist(5,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 beta 5: run <i><? echo createupgradelist(6,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 RC1: run <i><? echo createupgradelist(7,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 RC2: run <i><? echo createupgradelist(8,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0 RC3: run <i><? echo createupgradelist(9,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0.0: run <i><? echo createupgradelist(10,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0.1: run <i><? echo createupgradelist(11,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0.2: run <i><? echo createupgradelist(12,$scriptnumber); ?></i>
  <li>Upgrading from vBulletin 2.0.3: run <i><? echo createupgradelist(13,$scriptnumber); ?></i>
...
$check = $DB_site->query_first("SELECT optioncode FROM setting WHERE varname='highlightadmin'");

	echo "<b>Updating settings (fixing a typo) ....</b><br><br>";

	$DB_site->query("UPDATE setting SET description='Do you want a user to have to have the number of posts you specified above AND be registered the number of days you specified above? If you select NO then if a user satisfies EITHER criteria than they can use custom titles. For Ex a User has been registered for 1 year but only has 20 posts. If you specified 100 posts and 1 year then they could use titles if you select NO.' WHERE varname='ctEitherOr'");
	$DB_site->query("UPDATE setting SET description='Maximum number of options a user can select for the poll. Set this to 0 to allow infinitely many options.' WHERE varname='maxpolloptions'");
	$DB_site->query("UPDATE setting SET description='Add the template name at the beginning and end of every template rendered. This is useful for debugging and analyzing the HTML code, but turn it off to save bandwidth when running in a production environment.' WHERE varname='addtemplatename'");
	$DB_site->query("UPDATE setting SET description='The path that the cookie is saved to. If you run more than one board on the same domain, it will be necessary to set this to the individual directories of the forums. Otherwise, just leave it as / .'  WHERE varname='cookiepath'");
