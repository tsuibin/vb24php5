...
$check = $DB_site->query_first("SELECT optioncode FROM setting WHERE varname='highlightadmin'");
	...
	$DB_site->query("UPDATE setting SET description='On some servers, calls to getimagesize() may fail for currently unknown reasons. If you are affected by this, you will want to set this option to \"yes\". However, users may be able to get around your image dimension limits!',optioncode='yesno',displayorder='3' WHERE title='Allow Image Size Checking Failure?'");
	$DB_site->query("UPDATE usergroup SET title='Administrators' WHERE title='Administrator'");
	$DB_site->query("UPDATE usergroup SET title='Super Moderators' WHERE title='Super Moderator'");
